...
Development Track은 Phase 1~12까지 완료된 구현 트랙이며, Redis fallback, k6 측정, CI/CD Gate, Blue-Green 배포 검증까지 포함한다.
Ops Extension Track은 운영 관점의 장애 탐지, 복구, DR Drill, 보안 통제, Incident Runbook을 검증하는 별도 트랙이며 Phase 1~8까지만 진행한다.

## Project Scope

이 프로젝트는 두 개의 트랙으로 구성된다.

1. Development Track
   - 금융 이벤트 중복 처리, 상태 전이, PostgreSQL 정합성, Redis fallback, 성능 테스트, CI/CD Gate, Blue-Green 배포 검증을 다룬다.
   - Development Phase 1~12는 완료된 구현 트랙이다.

2. Ops Extension Track
   - 운영 관점에서 장애 탐지, 복구, DR Drill, 보안 통제, Incident Runbook을 검증한다.
   - Ops Phase는 1~8까지만 진행한다.
   - 현재 Ops Phase 7까지 완료했으며, Ops Phase 8 Incident Runbook finalization을 마지막 필수 작업으로 둔다.

| Track | Phase | Status | Main Evidence |
| --- | --- | --- | --- |
| Development | Phase 1~12 | Done | Backend consistency, Redis fallback, k6, CI/CD, Blue-Green |
| Ops | Phase 1~7 | Done | Metrics, DR Drill, failure simulation, alert, postmortem |
| Ops | Phase 8 | Planned / In Progress | Incident Runbook finalization |
| Supporting Docs | 27~34 | Supporting | Security, SLO/SLI, evidence template, capacity appendix |

## Scope Control

Ops 확장은 Phase 1~8까지만 수행한다.
...
범위 기준은 [Development Roadmap](./docs/04-development-roadmap.md), [Infra Operations Extension Plan](./docs/19-infra-operations-extension.md), [Incident Runbook Index](./docs/26-incident-runbook-index.md), [Blog Publishing Plan](./blog/README.md)에 연결한다.
...
- 선택적 운영 고도화: Ansible, PowerShell, Capacity Planning, Change Management
...
운영 증거 수집은 별도 계획으로 관리한다.
Prometheus target, Grafana dashboard, k6 summary, DR Drill report, 정합성 SQL 결과를 시나리오별로 저장하고,
README와 blog에는 대표 수치와 캡처만 요약한다.
...
기존 API custom metric 중심의 관측성을 Linux host, Docker container, PostgreSQL, Redis 계층까지 확장했다.
이번 단계는 실제 p95/p99/RPS 측정이 아니라 Prometheus/Grafana 기반 인프라 metric 수집 기반 구축이다.
...
PostgreSQL 백업 파일을 별도 restore DB에 복원하고, checksum/schema/consistency/sequence/RTO를 검증하는 DR Drill을 구성했다.

- Evidence report: `reports/dr/ops4-postgres-restore-drill.md`
- Runbook: `docs/22-postgres-backup-restore-drill.md`
- Blog: `blog/15-postgresql-backup-restore-drill.md`

### Ops Phase 5. Failure Recovery Runbook Drill

Redis, API, PostgreSQL 장애를 Docker Compose 환경에서 재현하고, 복구 후 health/ready/smoke/consistency check와 recovery duration을 report로 남겼다.

- Evidence report: `reports/ops/ops5-failure-recovery-drill.md`
- Runbook: `docs/23-failure-recovery-runbook-drill.md`
- Blog: `blog/20-failure-recovery-runbook-drill.md`

### Ops Phase 6. Alerting & Incident Response Runbook

Prometheus Alert Rule과 Incident Response Runbook을 구성하고, API/Redis/PostgreSQL/정합성 장애에 대한 탐지 기준과 1차 대응 절차를 문서화했다.

- Evidence report: `reports/ops/ops6-alerting-incident-runbook.md`
...
장애 탐지·대응·복구 검증·재발 방지 절차를 Incident Runbook Index와 supporting documents로 정리하는 마지막 필수 Ops 단계다.

- Runbook index: `docs/26-incident-runbook-index.md`
- Supporting docs: `docs/29-slo-sli-error-budget.md`, `docs/33-observability-evidence-plan.md`, `docs/34-measurement-result-template.md`
- Blog plan: `blog/README.md`

---

## 📚 기술 블로그 시리즈

이 프로젝트의 설계 과정과 구현 방식, 운영 확장 과정을 기술 블로그로 설명합니다.
추가 운영 확장 기획은 13편 이후 "금융 시스템 운영기"로 분리해 정리합니다.
...
13. [금융 거래 시스템에서 애플리케이션 메트릭만으로 부족했던 이유](./blog/13-why-infra-metrics-matter.md) - API, 서버, DB, Redis, Nginx 계층별 장애 원인 분리 기준을 설계한다.
14. [Nginx를 금융 이벤트 시스템의 운영 관문으로 설계하기](./blog/14-nginx-as-financial-ops-gateway.md) - 외부 이벤트 경로와 내부 운영 경로를 reverse proxy에서 분리한다.
15. [PostgreSQL 백업은 만들어지는 것보다 복구되는 것이 중요하다](./blog/15-postgresql-backup-restore-drill.md) - 백업, checksum, restore DB 복원, ledger/account 검증 흐름을 설계한다.
16. [Ansible로 금융 시스템 운영 작업 자동화하기](./blog/16-ansible-operation-automation.md) - 배포, 백업, 로그 수집, rollback을 idempotent playbook으로 표준화한다.
17. [Windows 운영자 단말에서 Linux/Docker 서비스를 점검하는 방법](./blog/17-windows-powershell-ops-check.md) - PowerShell 기반 health/readiness/metrics 점검과 incident snapshot을 설계한다.
18. [금융 사내망 관점에서 관리자 API와 Metrics 접근을 분리하기](./blog/18-internal-network-access-control.md) - public/internal endpoint 접근 정책과 로그 마스킹 기준을 정리한다.
...
- [Infra Operations Extension](./docs/19-infra-operations-extension.md)
- [Infra Metrics Design](./docs/20-infra-metrics-design.md)
- [Nginx Access Control](./docs/21-nginx-access-control.md)
- [Backup Restore Drill](./docs/22-backup-restore-drill.md)
- [PostgreSQL Backup Restore DR Drill](./docs/22-postgres-backup-restore-drill.md)
- [Failure Recovery Runbook Drill](./docs/23-failure-recovery-runbook-drill.md)
- [Alerting & Incident Response Runbook](./docs/24-alerting-incident-response-runbook.md)
- [Incident Timeline & Postmortem Drill](./docs/25-incident-timeline-postmortem-drill.md)
- [Incident Runbook Index](./docs/26-incident-runbook-index.md)
- [Optional Ansible Automation Design](./docs/23-ansible-automation-design.md)
- [Optional Windows PowerShell Ops](./docs/24-windows-powershell-ops.md)
- [Internal Network Security Supporting](./docs/25-internal-network-security.md)
...
- [Observability Evidence Plan](./docs/33-observability-evidence-plan.md)
- [Measurement Result Template](./docs/34-measurement-result-template.md)
