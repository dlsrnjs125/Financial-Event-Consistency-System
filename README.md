...
- `docs/22-postgres-backup-restore-drill.md`
- `blog/15-postgresql-backup-restore-drill.md`

로컬 evidence report에는 재현성을 위해 dump 파일 basename만 기록한다.
운영 환경에서는 파일명도 masking하거나 별도 backup id로 대체한다.

### Ops Phase 5. Failure Recovery Runbook Drill

Redis/API/PostgreSQL 장애를 Docker Compose 환경에서 재현하고, 복구 후
health/ready/smoke/consistency check를 실행한다.
PostgreSQL 장애 검증은 운영 DB volume 삭제 없이 `stop/start` 기반으로 수행하며,
복구 시간은 `reports/ops/ops5-failure-recovery-drill.md`에 count-only evidence로
기록한다.
...
- `blog/20-failure-recovery-runbook-drill.md`
