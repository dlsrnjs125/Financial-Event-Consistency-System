...
기존 API custom metric 중심의 관측성을 Linux host, Docker container, PostgreSQL,
Redis 계층까지 확장한다.
이번 단계는 실제 p95/p99/RPS 측정값을 기록하는 단계가 아니라,
Prometheus와 Grafana에서 인프라 metric을 수집하고 확인할 수 있는 운영 관측 기반을 만드는 단계다.
...
Nginx exporter는 `stub_status`와 내부 접근 제어 정책을 함께 설계해야 하므로
이번 단계에서는 dashboard TODO로 남기고 Ops Phase 2에서 보강한다.

로컬 개발 편의상 Prometheus/Grafana/exporter port를 host에 열지만,
운영 환경에서 `/metrics`와 exporter endpoint는 public endpoint가 아니다.
Prometheus/Grafana 접근은 내부 운영망 또는 인증된 운영자에게만 허용해야 한다.
PostgreSQL/Redis exporter는 host port를 열지 않고 Docker 내부 network에서 Prometheus가 수집한다.
...
실제 p95/p99, RPS, 장애 전후 비교 수치는 이후 k6/장애 재현 결과로 기록한다.
