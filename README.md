...
make final-check   # PR 전 format, lint, compile, test, security-log-check 실행
make ci-local      # Phase 11 CI Gate 로컬 대응 검증
...
모든 개발 작업은 PR 생성 전 `make final-check`로 포맷팅, 린트, Python 컴파일, 테스트, 민감 로그 검사를 확인한다.
...
| `alembic upgrade head` | `migration-tests` | PostgreSQL migration 검증 |
| `make security-log-check` | `security-log-check` | 민감 로그 검사 |
| `docker build -t financial-events:test ./backend` | `docker-build` | 배포 이미지 검증 |
| `make ci-local` | local equivalent | format/lint/test/security-log/compile 빠른 로컬 Gate |
