...
| Redis 없어도 정합성 유지 | Redis Down 후 중복 요청 | ✅ 0% 중복, 일부 5xx는 Phase 10 보완 |
...
현재는 `make final-check`, `make phase9-check`, `make k6-verify` 기반의 로컬 검증을 제공한다.
...
Phase 8의 Prometheus scrape 대상은 `prometheus`와 FastAPI `api-server`로 제한한다.
Redis/PostgreSQL 내부 지표는 exporter가 필요하므로 Phase 9 이후 부하 테스트/운영 관측 보강에서 추가한다.
