...
쉽다. 그래서 CI에서는 `MODE=help`와 `MODE=validate-report`를 실행해 script가 실제 CLI
모드로 동작하는지 확인하고, curated report 형식을 검증한다. 실제 Redis stop/start
incident drill evidence는 로컬 `make ops7-demo`로 남기는 방식으로 분리했다.
