...
모니터링은 장애를 알려준다. 하지만 장애가 발생했을 때 운영자가 어떤 순서로 확인하고, 어떤 기준으로 복구 완료를 판단할지는 별도의 문서가 필요하다.
...
Runbook은 실제 장애를 대신 해결하지 않는다. 하지만 장애 순간에 어떤 지표를 보고, 어떤 명령을 실행하고, 어떤 기준으로 복구를 선언할지 미리 정리해두면 대응 속도와 일관성이 크게 좋아진다.
