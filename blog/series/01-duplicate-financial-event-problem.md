...
## 왜 Kafka나 Queue부터 시작하지 않았나

실제 금융성 시스템에서는 외부 이벤트를 바로 DB에 반영하지 않고, 메시지 큐나 이벤트 스트림으로 수신과 처리를 분리할 수 있다. API는 이벤트를 안전하게 접수하고, 내부 consumer가 순서 보장, 재처리, DLQ, reconciliation을 담당하는 구조도 흔하다.

하지만 이 프로젝트의 첫 목표는 대규모 이벤트 파이프라인을 만드는 것이 아니라, "동일 금융 효과가 두 번 남지 않는가"를 가장 작은 구조에서 증명하는 것이었다.

Queue를 먼저 도입하면 `ACCEPTED`와 `COMPLETED`의 의미가 분리되고, consumer lag, replay, DLQ, eventual consistency까지 같이 설명해야 한다. 그러면 원장 반영 기준을 고정하기 전에 비동기 처리 의미부터 커진다.

그래서 의도적으로 동기 API, Idempotency Key, PostgreSQL transaction, unique constraint, ledger 검증을 먼저 구현했다. 성공 응답을 주려면 원장 반영 근거가 있어야 한다는 계약을 먼저 고정한 것이다.
