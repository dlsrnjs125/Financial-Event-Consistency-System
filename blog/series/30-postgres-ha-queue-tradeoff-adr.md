...
```text
client request
  -> durable queue append
  -> 202 ACCEPTED
  -> consumer processes event
  -> PostgreSQL ledger commit
  -> COMPLETED evidence
```
...
## 안 붙인 것이 아니라 먼저 판단한 것이다

PH8은 PostgreSQL HA와 durable queue를 부정한 단계가 아니다. 도입 전에 API contract, 정합성 책임, 운영 승인 경계를 ADR과 decision evidence로 먼저 정리한 단계다.

HA cluster, queue middleware, queue-first V2 API를 구현한 것은 아니다. 지금 구조에서 왜 direct transaction + fail-closed를 유지하는지 설명 가능하게 만든 것이 PH8의 산출물이다.
