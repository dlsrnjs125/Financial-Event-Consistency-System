...
## write suspend state를 파일로 둔 이유와 한계

이번 프로젝트에서는 write suspend 상태를 local runtime artifact로 남겼다. Docker Compose 기반 drill에서는 API process와 operator CLI가 같은 `reports/runtime` mount를 보게 만들면, resume 여부를 재현 가능하게 검증할 수 있다.

하지만 실제 운영에서 API instance가 여러 개라면 단순 파일 하나로는 부족하다. 어떤 instance는 resume 상태를 보고, 다른 instance는 아직 suspend 상태를 볼 수 있기 때문이다.

운영 환경에서는 보통 다음 중 하나의 별도 control plane이 필요하다.

- 중앙 설정 저장소
- feature flag / kill switch
- 관리형 coordination store
- 운영 승인 workflow
- instance별 config refresh와 propagation 확인
- 변경 이력 audit log

이번 프로젝트에서는 multi-instance write suspend propagation까지 구현하지 않았다. 대신 "DB commit 근거가 없으면 성공 응답을 주지 않는다"는 fail-closed 정책과, operator command와 API process가 같은 evidence를 봐야 한다는 문제를 local drill로 먼저 검증했다.
