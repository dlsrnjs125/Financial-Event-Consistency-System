...
이 글은 PH10 latency attribution과 PH11 latency drill evidence를 묶어, 성능 장애를 어떻게 계층별 후보로 좁혔는지 정리한다.
...
그래서 PH11 validator는 `ph10_input_scenario`로 evidence를 다시 만들고 PH10 analyzer를 재실행한다.

```text
stored actual classification
  vs
recomputed classification from PH10 analyzer
```
...
그래서 PH11 evidence에는 latency 숫자와 consistency counter를 함께 둔다.
...
PH10/PH11은 local sample evidence 기반의 attribution drill이다. 실제 운영 root cause 확정에는 tracing, DB lock view, exporter metric, network telemetry, external dependency SLA가 더 필요하다.
