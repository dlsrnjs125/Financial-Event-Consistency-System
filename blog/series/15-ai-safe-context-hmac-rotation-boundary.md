...
## sanitizer는 보안 완성이 아니라 사전 경계다

AI-safe context sanitizer가 있다고 해서 운영 데이터가 자동으로 안전해지는 것은 아니다. sanitizer는 외부 분석이나 AI 요약으로 넘기기 전에 context를 줄이는 마지막 방어선 중 하나다.

실제 운영에서는 그 앞뒤로 다음 통제가 필요하다.

- 원본 로그 수집 단계의 민감정보 최소화
- secret은 Vault/KMS 같은 전용 저장소에서 관리
- report 접근 권한 분리
- sanitizer policy review
- redaction test와 regression test
- 외부 AI API 사용 시 data processing agreement와 보존 정책 확인
- 감사 로그와 operator approval

이번 프로젝트에서는 외부 AI API를 호출하지 않았다. 목표는 AI를 붙이는 것이 아니라, AI에 넘길 수 있는 context와 절대 넘기면 안 되는 context의 경계를 먼저 정의하는 것이었다.
...
## nonce를 요구했지만 replay cache는 후속 범위다

`X-Nonce`를 필수로 요구한 것은 replay 방어 계약의 시작점이다. 하지만 nonce를 영구 저장하거나, partner별 nonce TTL을 관리하거나, 분산 환경에서 nonce 중복을 강하게 막는 replay cache까지 구현한 것은 아니다.

운영 환경에서는 nonce store, TTL, partner별 clock skew, key rotation window, replay cache eviction 정책이 함께 필요하다. 이번 프로젝트에서는 missing nonce reject와 signature evidence sanitization을 먼저 고정했다.
