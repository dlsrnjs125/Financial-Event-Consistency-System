...
| FinancialRedisDown | redis | Redis down 또는 readiness degraded 감지 |
...
| FinancialReconciliationFailureDetected | consistency | ledger/account 정합성 위반 의심 감지 |
...
Prometheus API rule load check가 SKIPPED이면 monitoring stack이 떠 있지 않은 상태다.
이 경우 `make ops6-up` 후 `make ops6-drill` 또는 `make ops6-demo`로 다시 확인한다.
