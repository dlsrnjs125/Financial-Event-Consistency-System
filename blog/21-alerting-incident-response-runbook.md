...
p95/p99 latency dashboard evidence는 [19편 Incident Runbook](./19-incident-runbook-oncall-simulation.md)에서 함께 정리했다.
...
`make ops6-demo` local alert validation, alert validation report, GitHub Actions
Ops6 gate evidence를 함께 남긴 것은 alert rule이 단순한 YAML 파일이 아니라
운영자가 믿고 사용할 수 있는 배포 Gate의 일부라는 점을 보여주기 위해서다.
