...
정합성의 최종 기준은 PostgreSQL이다.
Redis lock/cache, 애플리케이션 메모리, Prometheus metric은 정합성을 관측하거나 부하를 줄이는 보조 수단이며, 중복 거래 반영 여부는 PostgreSQL row와 Ledger/Account 검증으로 판단한다.
...
Phase 10 이후에는 Redis 장애 시에도 degraded mode로 DB 기준 처리를 계속한다.
반대로 PostgreSQL 장애는 Source of Truth 장애이므로 readiness failure로 다룬다.
