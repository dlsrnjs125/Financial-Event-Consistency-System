...
HMAC hash와 token은 원문은 아니지만 반복 사용 시 동일 주체 또는 동일 이벤트를 연결할 수 있는 pseudonymous identifier다.
따라서 AI context에는 필요한 최소 범위만 포함하고, 외부 공개 문서나 블로그 evidence에는 사용하지 않는다.
...
| AI context | allowlist 기반 Level 2~3 중심, Level 0 금지 |
...
AI context sanitizer는 raw field를 제거하는 denylist 방식이 아니라, 허용된 필드만 내보내는 allowlist 방식을 기본으로 한다.
...
## 11. Partner Secret Rotation & HMAC Hardening

PH Phase 6에서는 partner secret rotation과 replay defense를 별도 구현 후보로 관리한다.
이번 문서에서는 운영 설계 기준만 고정한다.

설계 기준:

- `X-Key-Id` 또는 secret version header로 사용 중인 partner secret version을 식별한다.
- `current` secret은 신규 요청 검증에 사용한다.
- `previous` secret은 짧은 rotation window 동안만 허용한다.
- `next` secret은 배포 전 검증과 staged rollout에만 사용하고, 실제 요청 검증에는 명시적 활성화 후 사용한다.
- revoked secret과 disabled client는 즉시 차단한다.
- timestamp skew 허용 범위를 넘은 요청은 replay risk로 거절한다.
- replay nonce 또는 `signature_id` 저장은 후속 구현에서 검토한다.
- secret rotation 최종 승인과 revoked secret 처리에는 운영자/보안 담당자 승인이 필요하다.

Rotation drill 성공 기준 후보:

- current/previous secret window에서 정상 요청이 통과한다.
- revoked secret 요청은 실패한다.
- disabled client 요청은 실패한다.
- raw secret, HMAC signature, Authorization header가 log/report/AI context에 남지 않는다.
- rotation evidence에는 key id/version과 masked client identifier만 남는다.
...
### 12.2 SHA-256 Hash vs HMAC Hash
...
### 12.3 randomized encryption vs deterministic encryption
...
### 12.5 pgcrypto vs KMS/Vault Transit
