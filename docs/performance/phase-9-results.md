...
| 날짜 | 시나리오 | 조건 | p95 | p99 | RPS | error rate | duplicate processing rate | ledger 중복 |
|------|----------|------|-----|-----|-----|------------|---------------------------|-------------|
| 2026-05-29 | smoke | Docker k6, 1 VU, 3 iterations | 83.01ms | 96.97ms | 1.89 req/s | 0.00% | 0.00% | N/A |
| 2026-05-29 | duplicate storm | Docker k6, 10 VUs, 5s | 10.60ms | 56.45ms | 168.07 req/s | 0.00% | 0.00% | 0건 |

초기 duplicate storm 검증 중 Nginx `limit_req zone="general"`이 503을 반환해 앱/Redis/DB 경로 측정을 가렸다.
Phase 9 로컬 실험에서는 `infra/nginx/nginx.conf`의 rate limit을 높여 duplicate storm이 애플리케이션까지 도달하도록 조정했다.
...
| redis down | `docker compose pause redis && make k6-redis-down && docker compose unpause redis` | TBD |
...
| VUs | duration | p50 | p95 | p99 | 200 | 202 | 409 | error rate | duplicate processing rate | ledger 중복 |
|-----|----------|-----|-----|-----|-----|-----|-----|------------|---------------------------|-------------|
...
```bash
docker compose exec postgres psql -U postgres -d financial_events
```

`tests/k6/sql/verify-consistency.sql`의 쿼리를 실행해 동일 `external_event_id`에 대해 `ledger_entries`가 2건 이상 생성되지 않았는지 확인한다.

## Redis Down 결과

| VUs | duration | p50 | p95 | p99 | RPS | error rate | redis unavailable | duplicate processing rate | ledger 중복 |
|-----|----------|-----|-----|-----|-----|------------|-------------------|---------------------------|-------------|
...
Redis 장애 중에도 PostgreSQL 기준 duplicate processing rate는 0%여야 한다.

## Redis Cache 사용 전/후 비교

| 조건 | p50 | p95 | p99 | RPS | error rate | cache hit ratio | DB lookup/transaction 지표 | duplicate processing rate |
|------|-----|-----|-----|-----|------------|-----------------|----------------------------|---------------------------|
...
| 조건 | p50 | p95 | p99 | RPS | error rate | lock rejected | DB transaction count | duplicate processing rate |
|------|-----|-----|-----|-----|------------|---------------|----------------------|---------------------------|
