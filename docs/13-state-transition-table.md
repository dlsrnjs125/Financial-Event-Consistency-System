...
## 5. Terminal Status 기준

Phase 3 기준에서는 더 이상 허용된 다음 전이가 없는 `FAILED`, `CANCELLED`, `SETTLED`를 terminal status로 간주한다.

단, `FAILED` 재처리 정책이 필요해질 경우 별도 `RETRY` 이벤트 또는 `FAILED -> PROCESSING` 전이를 후속 ADR로 검토한다.
