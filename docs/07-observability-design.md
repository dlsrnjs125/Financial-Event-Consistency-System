...
## Ops Phase 6 기준 Alerting 정리

- Alert rule은 `infra/prometheus/rules/financial_event_alerts.yml`을 canonical source로 둔다.
- API down, 5xx error rate, p95 latency, Redis down/degraded, PostgreSQL down, invalid state transition, reconciliation failure를 required alert inventory로 관리한다.
- Redis down은 PostgreSQL 기준 정합성 실패가 아니므로 warning으로 분류한다.
- PostgreSQL down과 reconciliation failure는 Source of Truth 또는 금융 정합성 위험이므로 critical로 분류한다.
- Alertmanager/Slack/PagerDuty 실연동은 이번 Phase에서 제외하고, rule 문법과 runbook 연결을 CI/local evidence로 검증한다.
...
현재 구현에는 PostgreSQL exporter가 포함되지 않는다.
아래 패널은 Phase 12 이후 exporter 또는 SQLAlchemy pool gauge를 추가한 뒤 활성화한다.
...
현재 구현에는 Redis exporter가 포함되지 않는다.
애플리케이션에서 노출하는 Redis lock/cache/fallback 도메인 메트릭을 우선 사용하고, Redis 서버 내부 지표는 Phase 12 이후 운영 관측 보강에서 추가한다.
...
Phase 12에서는 Prometheus exporter를 추가하지 않고, `make deploy-status`와 deployment script 로그로 active upstream, Blue/Green container 상태, 배포 전후 smoke 결과를 확인한다.
Prometheus에서는 API가 노출하는 `financial_http_errors_total`, `financial_http_request_duration_seconds`, `financial_invalid_state_transition_total`, `financial_reconciliation_failures_total`, `financial_redis_fallback_total`, `financial_readiness_dependency_status`를 배포 전후 비교한다.
