...
아래 패널은 Phase 11 이후 exporter 또는 SQLAlchemy pool gauge를 추가한 뒤 활성화한다.
...
애플리케이션에서 노출하는 Redis lock/cache/fallback 도메인 메트릭을 우선 사용하고, Redis 서버 내부 지표는 Phase 11 이후 운영 관측 보강에서 추가한다.
