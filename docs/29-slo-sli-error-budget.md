...
PH10 latency attribution analyzer는 p95/p99 spike를 SEV2 후보 증상으로 취급하고, Nginx/FastAPI/Redis/PostgreSQL/outbound/blackbox/consistency evidence를 함께 비교해 원인 후보를 분류한다.
다만 duplicate ledger, duplicate external event, reconciliation failure, invalid state transition이 함께 발견되면 latency warning보다 consistency incident boundary를 우선한다.
PH11 latency drill evidence runner도 같은 경계를 따른다. Local latency drill threshold는 production SLO가 아니며, drill 중 consistency counter가 non-zero이면 latency issue가 아니라 SEV1 consistency incident 후보로 먼저 다룬다.
