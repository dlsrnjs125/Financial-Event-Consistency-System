...
Phase 8은 측정 기반을 만드는 단계였고, 실제 p95/p99/RPS/error rate 수치는 Phase 9 k6 실험에서 기록했다.
Phase 10에서는 Redis fallback, DB conflict retry, readiness dependency 상태 metric을 추가했다.
기존 `http_requests_total`, `http_request_duration_seconds`는 Phase 1부터 제공한 기본 HTTP metric으로 유지한다.
현재 표준 관측 기준은 bounded label을 사용하는 `financial_*` metric이다.
...
Grafana dashboard provisioning과 Prometheus datasource provisioning은 로컬 검증 편의를 위한 구성이다.
현재 Prometheus scrape 대상은 앱 도메인 메트릭에 집중하기 위해 `prometheus`와 `api-server`로 제한한다.
...
실제 운영 임계값과 대시보드 패널 조정은 Phase 9/10 로컬 측정값을 기준선으로 삼고, 장시간 운영 데이터가 쌓인 뒤 확정한다.
