...
- Blue-Green vs Canary: Canary처럼 점진적 트래픽 실험은 어렵지만 로컬 Docker Compose 환경에서 절차를 명확히 재현할 수 있다.
...
- DB migration rollback을 자동화하지 않은 이유
...
- Redis degraded 상태에서 배포를 허용한 이유
