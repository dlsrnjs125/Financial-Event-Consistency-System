...
| PH Phase 7 | Partner Secret Rotation & HMAC Hardening 구현 | `39-sensitive-data-ai-governance-and-encryption-tradeoff.md`, `docs/28-secret-management-policy.md`, `49-ph7-partner-secret-rotation-hmac-hardening.md` | Drafted | Implemented |
| PH Phase 8 | PostgreSQL HA / Durable Queue Trade-off ADR | `40-postgres-ha-and-queue-tradeoff-adr.md`, `50-ph8-postgres-ha-queue-decision-evidence.md`, `reports/architecture/ph8-ha-queue-tradeoff/` | Drafted | Implemented |
| PH Phase 9 | Production Hardening Drill Plan & Evidence Runner | `51-ph9-production-hardening-drill-plan.md`, `reports/production-hardening/ph9-drill-plan/` | Drafted | Implemented |
| PH Phase 10 | Latency Attribution / External Dependency Diagnosis 구현 | `41-latency-attribution-and-external-dependency-diagnosis.md`, `52-ph10-latency-attribution-diagnosis.md`, `reports/latency/ph10-attribution/` | Drafted | Implemented |
| PH Phase 11 | Latency Drill Test Plan & Safe Evidence Runner 구현 | `42-latency-drill-test-plan.md`, `53-ph11-latency-drill-evidence-runner.md`, `reports/latency/ph11-drill-evidence/` | Drafted | Implemented |
...
| PH-Impl 7 | 완료 | partner secret version verifier, HMAC rotation drill, sanitized evidence | 외부 partner 인증 rotation contract와 민감정보 미노출 검증 |
| PH-Impl 8 | 완료 | PostgreSQL HA / durable queue trade-off ADR, decision matrix generator, sanitized architecture evidence | API 응답 의미와 정합성 책임 변화 설명 |
| PH-Impl 9 | 완료 | PH1~PH8 hardening drill catalog, safe evidence runner, report validator | 운영 drill 순서와 자동화/수동 승인 경계를 재현 가능한 evidence로 묶기 |
| PH-Impl 10 | 완료 | deterministic latency attribution analyzer, sanitized sample evidence/report, validator | k6 증상을 Nginx/FastAPI/Redis/PostgreSQL/outbound/blackbox/consistency evidence와 묶어 원인 후보 분류 |
| PH-Impl 11 | 완료 | LAT-001~LAT-006 safe latency drill catalog, PH10 input evidence generator, expected/actual validator | latency 증상 재현 계획과 server/consistency evidence를 PH10 analyzer에 연결 |
| PH-Impl 12 | 선택 | persistent nonce store, Vault/KMS 연동 ADR | PH7 rotation contract 이후 운영 통합 후보 |
| PH-Impl 13 | 선택 | HA/Queue PoC 여부 결정 | PH8 decision evidence 이후 별도 V2/production candidate로 판단 |
...
PH-Impl 7 partner HMAC rotation 실행 방법은 [49-ph7-partner-secret-rotation-hmac-hardening.md](49-ph7-partner-secret-rotation-hmac-hardening.md)를 기준으로 관리한다.
PH-Impl 8 HA/Queue trade-off evidence 실행 방법은 [50-ph8-postgres-ha-queue-decision-evidence.md](50-ph8-postgres-ha-queue-decision-evidence.md)를 기준으로 관리한다.
PH-Impl 9 production hardening drill catalog와 evidence runner 실행 방법은 [51-ph9-production-hardening-drill-plan.md](51-ph9-production-hardening-drill-plan.md)를 기준으로 관리한다.
...
- latency attribution candidate report 생성
...
- 대표 트러블슈팅 서사는 `blog/series/24-*` ~ `blog/series/33-*` 글에서 관리한다.
