...
| PH-Impl 11 | 선택 | persistent nonce store, Vault/KMS 연동 ADR | PH7 rotation contract 이후 운영 통합 후보 |
| PH-Impl 12 | 선택 | HA/Queue PoC 여부 결정 | PH8 decision evidence 이후 별도 V2/production candidate로 판단 |
...
PH-Impl 10은 latency evidence analyzer 구현 단계이며, PH11 k6 latency drill/fault injection 실행 완료를 의미하지 않는다.
