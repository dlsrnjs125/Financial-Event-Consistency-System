...
## 2. Evidence Required for Ops Phase 8

| Scenario | Required Evidence | Source |
| --- | --- | --- |
| Redis Down / Redis Degraded | fallback metric, readiness degraded, duplicate ledger count 0, recovery result | Grafana / logs / terminal / `reports/ops/` |
| PostgreSQL Connection Exhausted | DB connection panel, readiness failure, error logs, recovery check | Grafana / PostgreSQL / logs |
| Nginx 5xx Spike | 5xx panel, access log sample, upstream status | Grafana / Nginx logs |
| High Latency | p95/p99 panel, request duration histogram, dependency latency clue | Grafana / Prometheus |
| Failed Deployment / Rollback | blue/green status, rollback command result, routed smoke result | terminal / deployment logs |
| Consistency Violation | violation count, duplicate prevention evidence, reconciliation result | DB query / application logs / report |
| Secret Leak / Security Incident | masked log evidence, secret scan result, security checklist result | logs / CI / security checklist |

실제 이미지를 이 문서에 직접 추가하지 않아도 된다.
대신 블로그나 README에 이미지를 넣기 전, 위 표의 scenario/source/evidence가 맞는지 확인한다.
