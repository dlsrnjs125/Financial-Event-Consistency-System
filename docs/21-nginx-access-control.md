...
Public Nginx는 host `8080`에서 접근한다. Public Nginx는 denylist가 아니라 allowlist 방식으로 구성한다.
...
무서명 `POST /api/v1/transaction-events` 요청은 기존 HMAC 정책에 따라 실패해야 한다. Nginx access control은 HMAC을 대체하지 않는다.
