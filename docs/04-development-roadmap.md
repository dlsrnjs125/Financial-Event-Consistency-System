...
- 현재 위치: **Production Hardening PH5 구현 및 검증**
- GitHub 초기 Push: **완료**
- 다음 단계: **PH-Impl 6 AI-safe Context Sanitizer 범위 검토**
...
| 진행 | Production Hardening Track | 운영 장애 판단, 데이터 보호, 복구 승인 절차 설계와 PH1~PH5 안전장치 구현 | docs/35~47, runbook, write suspend service/script, incident artifact/analyzer/recovery case/reconciliation script | stale detector와 count-only reconciliation 결과를 recovery case로 연결 가능 |
...
- README에는 요약과 링크만 두고, 상세 정책은 `docs/35-*` ~ `docs/47-*`에서 관리한다.
