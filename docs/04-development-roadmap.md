...
## Phase 12~18: Infra Operations Extension

기존 Phase 1~12가 금융 이벤트 정합성, Redis fallback, k6 부하 테스트, Prometheus/Grafana, CI/CD Gate, Blue-Green/Rollback을 검증하는 단계였다면, 추가 운영 확장 단계는 동일 시스템을 실제 금융권 사내 인프라에서 운영한다는 가정으로 확장한다.

이 로드맵은 기존 구현 Phase 12를 대체하지 않는다. Blue-Green/Rollback까지 완료된 시스템 위에 운영 관측, 접근 제어, 백업/복구, 자동화, Windows 운영자 점검, 내부망 통제, 장애 Runbook을 추가로 설계하는 기획 단계다.
...
### Phase 12. Infra Metrics Extension
...
### Phase 13. Nginx Reverse Proxy & Access Control

- 외부 이벤트 API와 내부 운영 API 접근 경로 분리
- `/metrics`, `/ready`, `/admin/*` 접근 제한
- rate limit, security headers, upstream latency log 추가
- 완료 기준: `make nginx-test`, `make rate-limit-test`, `make admin-access-test`

### Phase 14. Backup, Restore, DR Drill
...
### Phase 15. Ansible Operation Automation
...
### Phase 16. Windows/PowerShell Operator Scripts
...
### Phase 17. Internal Network & Secure Admin Access
...
### Phase 18. Incident Runbook & On-call Simulation
