...
- 현재 위치: **Phase 2 데이터 모델과 Migration 구현 완료**
- GitHub 초기 Push: **완료**
- 다음 단계: **Phase 3 상태 머신 구현**

Phase 2 완료 범위에는 정합성 기준 ORM 모델, PostgreSQL Unique Constraint, 주요 조회 Index, Alembic 초기 Migration, 모델 metadata 검증 테스트가 포함된다.
실제 거래 이벤트 처리 로직, 상태 전이 처리, Idempotency 응답 재사용, Redis Lock/Cache는 다음 Phase에서 순차적으로 구현한다.
...
| 다음 | 3. 상태 머신 구현 | 허용/금지 상태 전이 구현 | 상태 enum, 전이표, 이력 저장 | 상태 머신 Unit Test 통과 |
...
- 현재 백엔드는 Phase 2 데이터 모델과 Migration 구현 완료 상태이므로 거래 이벤트 API의 실제 처리 로직은 Phase 3 이후 구현한다.
- Consistency Test 파일은 테스트 시나리오 골격을 먼저 잡은 상태이며, Transaction/Idempotency 처리 구현 후 skip을 제거한다.
- Phase 3에서는 상태 enum, 허용 전이표, InvalidStateTransition 예외, EventStateHistory 기록 로직을 구현한다.
- Phase 3 완료 후 상태 머신 Unit Test를 실제 앱 코드 기준으로 통과시키고, 이 문서의 Phase 3 상태를 `완료`로 변경한다.
