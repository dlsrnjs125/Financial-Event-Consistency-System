...
- 현재 위치: **Production Hardening Track 기획 문서 작성**
- GitHub 초기 Push: **완료**
- 다음 단계: **Production Hardening 구현 후보 검토 및 PR 정리**
...
Production Hardening Track에서는 PostgreSQL 자체 장애, write suspend/resume, incident diagnosis automation, recovery case/quarantine/reconciliation, AI-safe sensitive data governance, PostgreSQL HA와 durable queue trade-off를 설계 문서로 정리한다.
...
| 진행 | Production Hardening Track | 운영 장애 판단, 데이터 보호, 복구 승인 절차 설계 | docs/35~40, runbook skeleton | 이번 브랜치에서는 설계 문서 작성만 완료 |
...
- README에는 요약과 링크만 두고, 상세 정책은 `docs/35-*` ~ `docs/40-*`에서 관리한다.
