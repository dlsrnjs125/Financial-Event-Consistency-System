...
- 현재 위치: **Phase 0 설계 기준 확정 및 초기 세팅 완료**
- GitHub 초기 Push: **완료**
- 다음 단계: **Phase 1 백엔드 프로젝트 골격 구현**
...
| 다음 | 1. 백엔드 프로젝트 골격 구현 | FastAPI 앱 구조 구성 | 앱 구조, 설정, DB/Redis 연결, Health/Ready | Docker Compose 환경에서 API 실행 |
| 대기 | 2. 데이터 모델과 Migration 구현 | 정합성 기준 DB 모델 구현 | ORM 모델, Alembic Migration, Unique/Index | Migration 성공, 제약조건 테스트 통과 |
| 대기 | 3. 상태 머신 구현 | 허용/금지 상태 전이 구현 | 상태 enum, 전이표, 이력 저장 | 상태 머신 Unit Test 통과 |
| 대기 | 4. Idempotency 처리 구현 | 재시도/충돌 요청 구분 | request_hash, IdempotencyRecord, 응답 재사용 | 같은 Key/Body 재요청 기존 응답 반환 |
| 대기 | 5. Transaction과 Ledger 처리 구현 | 거래 반영 원자성 보장 | TransactionEventService, LedgerService, Row Lock | 동일 이벤트 100회 요청 시 Ledger 1건 |
...
- [ ] FastAPI 앱 실행 가능
- [ ] Router / Service / Repository / Domain 폴더 구조 생성
- [ ] settings/config 모듈 작성
- [ ] PostgreSQL 연결 확인
- [ ] Redis 연결 확인
- [ ] `/health` 구현
- [ ] `/ready` 구현
...
- [ ] accounts 테이블 구현
- [ ] transaction_events 테이블 구현
- [ ] ledger_entries 테이블 구현
- [ ] idempotency_records 테이블 구현
- [ ] event_state_histories 테이블 구현
- [ ] external_event_id UNIQUE 적용
- [ ] idempotency_key UNIQUE 적용
- [ ] ledger_entries.transaction_event_id UNIQUE 적용
- [ ] 주요 조회 컬럼 Index 적용
- [ ] Alembic upgrade 성공
- [ ] Migration 테스트 추가
...
- [ ] 허용 상태 전이표 작성
- [ ] 상태 전이 검증 함수 작성
- [ ] InvalidStateTransition 예외 작성
- [ ] EventStateHistory 저장
- [ ] 상태 머신 단위 테스트 작성
- [ ] 금지 상태 전이 테스트 작성
- [ ] CANCEL 관련 상태 테스트 작성
...
- [ ] Idempotency-Key Header 검증
- [ ] request_hash 생성 로직 작성
- [ ] IdempotencyRecord Repository 작성
- [ ] 동일 요청 재전송 처리
- [ ] 충돌 요청 409 처리
- [ ] 처리 중 요청 202 처리
- [ ] 완료 응답 재사용 처리
...
- `lock:idempotency:{key}` 설계
- `cache:idempotency:{key}` 설계
...
- Redis Lock 획득 실패 시 중복 처리 위험이 줄어든다.
...
- [ ] Redis 연결 구현
- [ ] RedisLockManager 작성
- [ ] Idempotency Cache 저장/조회 구현
- [ ] TTL 정책 적용
- [ ] Redis Down fallback 구현
- [ ] Redis Cache 전후 성능 비교
- [ ] Redis Lock 전후 DB transaction 수 비교
- [ ] Redis Down 테스트 작성
...
- `financial_hmac_signature_invalid_total`
...
- [ ] `/metrics` 구현
- [ ] API latency metric 추가
- [ ] 이벤트 수신 metric 추가
- [ ] 중복 이벤트 metric 추가
- [ ] 상태 전이 실패 metric 추가
- [ ] reconciliation 실패 metric 추가
- [ ] 구조화 로그 적용
- [ ] trace_id/request_id 적용
- [ ] Grafana Dashboard 초안 작성
