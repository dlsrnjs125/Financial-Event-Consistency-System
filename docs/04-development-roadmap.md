...
- 현재 위치: **Production Hardening PH9 구현 및 검증**
- GitHub 초기 Push: **완료**
- 다음 단계: **PH9 CI 검증 및 PR 리뷰**
...
| 진행 | Production Hardening Track | 운영 장애 판단, 데이터 보호, 복구 승인 절차 설계와 PH1~PH9 안전장치/evidence 구현 | docs/35~51, runbook, write suspend service/script, incident artifact/analyzer/recovery case/reconciliation/AI context/HMAC rotation/HA queue decision/drill catalog script | sanitized evidence, HMAC rotation contract, HA/Queue trade-off, PH9 drill catalog 검증 가능 |
...
- README에는 요약과 링크만 두고, 상세 정책은 `docs/35-*` ~ `docs/51-*`에서 관리한다.
...
| PH-Impl 10 | 선택 | FastAPI phase timer, Nginx timing log parser | latency attribution 구현 기반 |
...
### PH Phase 10. Latency Attribution / External Dependency Diagnosis 설계
