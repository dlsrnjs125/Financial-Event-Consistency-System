...
- 현재 위치: **Phase 6 Redis Lock/Cache 적용 완료**
- GitHub 초기 Push: **완료**
- 다음 단계: **Phase 7 HMAC 보안 강화**

Phase 6 완료 범위에는 Redis 기반 Idempotency Lock, 완료 응답 Cache, CachedIdempotencyService, Redis 장애 시 DB fallback, Redis Down 상태에서도 동일 이벤트 중복 반영 0건을 확인하는 회귀 테스트가 포함된다.
Redis는 중복 요청 폭주 완화와 응답 재사용을 위한 최적화 계층이며, PostgreSQL Unique Constraint와 DB Transaction이 최종 정합성 기준이다.
HMAC 인증, 도메인 메트릭 본격화, k6 부하 테스트는 다음 Phase에서 순차적으로 구현한다.
...
| 다음 | 7. 보안 처리 구현 | 외부 시스템 인증/변조 검증 | HMAC, Timestamp, Secret 관리, 로그 마스킹 | 잘못된 Signature/Timestamp 차단 |
