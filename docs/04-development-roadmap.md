...
PH5에서는 stale `PROCESSING` idempotency record와 count-only reconciliation issue를 탐지해 recovery case로 연결하고 sanitized report artifact를 생성한다.
PH6에서는 PH2~PH5 evidence를 AI나 외부 분석 도구에 전달하기 전 allowlist 기반 AI-safe context로 변환한다.
PH7에서는 partner secret rotation 중 `current`/`previous`/`next`/`revoked`/`disabled` HMAC 검증과 sanitized rotation evidence를 구현한다.
...
| PH-Impl 3 | 완료 | Incident Analyzer MVP | 현재 사용 가능한 `/ready`, app metric, consistency SQL 기반 자동 분류 |
| PH-Impl 4 | 완료 | `recovery_cases` migration, recovery case service, approval status | 차단 이후 복구 흐름 구현 |
| PH-Impl 5 | 완료 | stale PROCESSING detector, reconciliation 확장 | 미확정 거래 처리 |
| PH-Impl 6 | 완료 | AI-safe context sanitizer | 장애 분석/AI 활용 전 데이터 보호 |
| PH-Impl 7 | 완료 | partner secret version/rotation drill | 보안 운영 고도화 |
...
- [x] stale detector 구현
- [x] reconciliation job 확장
- [x] [47-ph5-stale-processing-reconciliation.md](47-ph5-stale-processing-reconciliation.md)에 PH5 구현 기록 추가

**연결 문서**

- [38-recovery-case-quarantine-and-reconciliation-design.md](38-recovery-case-quarantine-and-reconciliation-design.md)
- [47-ph5-stale-processing-reconciliation.md](47-ph5-stale-processing-reconciliation.md)
- [12-data-model-spec.md](12-data-model-spec.md)
...
- compensation ledger/manual correction workflow

### PH Phase 6. Sensitive Data Protection & AI Safe Governance 구현
...
- [x] AI context sanitizer 구현
- [x] [48-ph6-ai-safe-context-sanitizer.md](48-ph6-ai-safe-context-sanitizer.md) 작성
...
- [48-ph6-ai-safe-context-sanitizer.md](48-ph6-ai-safe-context-sanitizer.md)
...
- PH6 AI-safe context sanitizer 운영 확장
- HMAC lookup hash migration
- KMS/Vault Transit ADR

### PH Phase 7. Partner Secret Rotation & HMAC Hardening 구현
...
- [x] partner secret version verifier 구현
- [x] rotation drill 구현
- [x] sanitized rotation report 검증
...
- [28-secret-management-policy.md](28-secret-management-policy.md)
- [39-sensitive-data-ai-governance-and-encryption-tradeoff.md](39-sensitive-data-ai-governance-and-encryption-tradeoff.md)
- [49-ph7-partner-secret-rotation-hmac-hardening.md](49-ph7-partner-secret-rotation-hmac-hardening.md)

**후속 구현 후보**

- Vault/KMS/Secret Manager 연동
- persistent nonce store
- production key retirement runbook

### PH Phase 8. PostgreSQL HA / Durable Queue Trade-off ADR
...
- deterministic decision matrix generator와 validator 구현

**검증 기준**

- queue 도입 시 API 계약이 처리 완료가 아니라 수신 완료로 바뀐다는 점이 명확하다.
- HA를 사용해도 write resume 전 정합성 검증이 필요하다는 점이 명확하다.
- generated report가 required option, score range, 민감정보 미포함, 금지 claim 검증을 통과한다.
...
- [x] [50-ph8-postgres-ha-queue-decision-evidence.md](50-ph8-postgres-ha-queue-decision-evidence.md) 작성
- [x] `scripts/ph8_ha_queue_decision_matrix.py` 구현
- [x] sample decision report 생성
- [ ] HA lab 구현 여부 결정
- [ ] queue-first API V2 contract 초안 작성

**연결 문서**

- [40-postgres-ha-and-queue-tradeoff-adr.md](40-postgres-ha-and-queue-tradeoff-adr.md)
- [50-ph8-postgres-ha-queue-decision-evidence.md](50-ph8-postgres-ha-queue-decision-evidence.md)
- [36-postgres-failure-and-write-suspend-policy.md](36-postgres-failure-and-write-suspend-policy.md)
- [15-api-contract.md](15-api-contract.md)
...
### PH Phase 9. Production Hardening Drill Plan & Evidence Runner

**목표**

PH1~PH8 hardening 산출물을 실제 운영 drill 관점에서 하나의 catalog와 evidence report로 묶고, 자동화 가능한 검증과 사람이 승인해야 하는 작업의 경계를 명확히 한다.

**주요 작업**

- PH1~PH8 drill catalog 정의
- safe auto-run과 manual-run boundary 분리
- 실제 존재하는 Makefile command만 default command로 기록
- destructive/manual action은 candidate 또는 manual approval boundary로 분리
- PH10/PH11 latency work는 follow-up candidate로만 연결
- JSON/Markdown sample report 생성
- validator와 unit test 추가

**검증 기준**

- PH1~PH8 drill이 모두 포함되어 있다.
- PH10/PH11 latency drill이 completed drill로 표시되지 않는다.
- destructive/manual action이 default `commands`에 들어가지 않는다.
- safe auto-run drill에 write resume, failover promote, ledger correction 같은 승인 작업이 포함되지 않는다.
- 민감 runtime data가 report에 포함되지 않는다.
...
- manual approval boundary violation count

**완료 체크리스트**

- [x] [51-ph9-production-hardening-drill-plan.md](51-ph9-production-hardening-drill-plan.md) 작성
- [x] `scripts/ph9_production_hardening_drill.py` 구현
...
- [51-ph9-production-hardening-drill-plan.md](51-ph9-production-hardening-drill-plan.md)
- [35-production-hardening-roadmap.md](35-production-hardening-roadmap.md)
...
### PH Phase 10. Latency Attribution / External Dependency Diagnosis 구현

**목표**

외부 시스템이 "API가 느리다"고 신고했을 때 내부 처리, Nginx edge/network, Redis/PostgreSQL, outbound external dependency 중 어느 구간의 지연인지 좁히는 deterministic analyzer와 sanitized report를 구현한다.
...
- `scripts/ph10_latency_attribution.py` analyzer 구현
- sanitized sample input/report JSON/Markdown 생성
- validator와 unit test 추가
- Makefile `ph10-latency-*` target 추가
...
- PH11 latency drill/fault injection 완료를 PH10에서 주장하지 않는다.
...
- [x] [52-ph10-latency-attribution-diagnosis.md](52-ph10-latency-attribution-diagnosis.md) 작성
- [x] [37-incident-diagnosis-automation-design.md](37-incident-diagnosis-automation-design.md)에 LAT rule 연결
- [x] `scripts/ph10_latency_attribution.py` 구현
- [x] sample latency attribution JSON/Markdown report 생성
- [x] PH10 validator와 unit test 추가
- [x] Makefile `ph10-latency-*` target 추가
- [ ] PH11 phase latency instrumentation 구현
- [ ] latency attribution dashboard 구현

**연결 문서**

- [41-latency-attribution-and-external-dependency-diagnosis.md](41-latency-attribution-and-external-dependency-diagnosis.md)
- [52-ph10-latency-attribution-diagnosis.md](52-ph10-latency-attribution-diagnosis.md)
- [37-incident-diagnosis-automation-design.md](37-incident-diagnosis-automation-design.md)
- [33-observability-evidence-plan.md](33-observability-evidence-plan.md)
