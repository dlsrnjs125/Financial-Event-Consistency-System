...
- 현재 위치: **Phase 3 상태 머신 구현 완료**
- GitHub 초기 Push: **완료**
- 다음 단계: **Phase 4 Idempotency 처리 구현**

Phase 3 완료 범위에는 거래 이벤트 상태 enum, 허용 상태 전이표, 금지 상태 전이 검증, InvalidStateTransition 예외, EventStateHistory 저장 기반, CANCEL 가능 상태 검증 테스트가 포함된다.
실제 거래 이벤트 수신 API, Idempotency 응답 재사용, Account balance 변경, Ledger 생성, Redis Lock/Cache는 다음 Phase에서 순차적으로 구현한다.
...
| 다음 | 4. Idempotency 처리 구현 | 재시도/충돌 요청 구분 | request_hash, IdempotencyRecord, 응답 재사용 | 같은 Key/Body 재요청 기존 응답 반환 |
...
- 현재 백엔드는 Phase 3 상태 머신 구현 완료 상태이므로 거래 이벤트 API의 실제 처리 로직은 Phase 4 이후 구현한다.
- Consistency Test 파일은 테스트 시나리오 골격을 먼저 잡은 상태이며, Transaction/Idempotency 처리 구현 후 skip을 제거한다.
- Phase 4에서는 Idempotency-Key 검증, request_hash 생성, IdempotencyRecord repository, 동일 요청 응답 재사용, 충돌 요청 409 처리를 구현한다.
- Phase 4 완료 후 Idempotency Unit Test와 Service Test를 실제 앱 코드 기준으로 통과시키고, 이 문서의 Phase 4 상태를 `완료`로 변경한다.
