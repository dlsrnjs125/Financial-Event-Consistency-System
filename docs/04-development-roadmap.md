...
- 현재 위치: **Production Hardening PH8 구현 및 검증**
- GitHub 초기 Push: **완료**
- 다음 단계: **PH8 CI 검증 및 PR 리뷰**
...
| 진행 | Production Hardening Track | 운영 장애 판단, 데이터 보호, 복구 승인 절차 설계와 PH1~PH8 안전장치 구현 | docs/35~50, runbook, write suspend service/script, incident artifact/analyzer/recovery case/reconciliation/AI context/HMAC rotation/HA queue decision script | sanitized evidence, HMAC rotation contract, HA/Queue trade-off evidence 검증 가능 |
...
- README에는 요약과 링크만 두고, 상세 정책은 `docs/35-*` ~ `docs/48-*`에서 관리한다.
...
### PH Phase 9. Production Hardening Drill 구현 계획
