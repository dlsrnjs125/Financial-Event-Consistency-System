...
| FinancialRedisDown | `redis_up == 0` 또는 Redis readiness degraded | warning | cache/lock 성능 저하 | PostgreSQL 기준 정합성은 유지되어야 함 | fallback metric, DB retry, Ops5 Redis drill 연결 |
...
| FinancialReconciliationFailureDetected | reconciliation failure non-zero | critical | 서비스 중단 검토 | 금융 정합성 사고 가능성 | 트래픽 제한, evidence 보존, 정합성 SQL 실행 |
...
- CI에서 Prometheus API load check를 제외한 것은 의도적 trade-off다. 실제 rule load evidence는 `reports/ops/ops6-alerting-incident-runbook.md`에 로컬 실행 결과로 남긴다.
